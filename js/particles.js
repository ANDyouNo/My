...
function createParticle (x, y, type) {
  ...
  switch (type) {
      case 'square':
      particle.style.background = `hsl(${Math.random() * 50 + 300}, 85%, 52%)`; // Цвет квадратов
      particle.style.border = '1px solid white'; // Бордюр квадратов
      break;
      case 'shadow':
      var color = `hsl(${Math.random() * 50 + 300}, 85%, 50%)`; // Цвет 
...
if (document.body.animate) {
  document.querySelector('.header__button').addEventListener('click', pop);
}